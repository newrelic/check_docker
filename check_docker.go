...
func main() {
	...
	flag.Var(&imageIds, "image-id", "An image ID that must be running on the Docker server (can specify multiple times)")
	flag.Var(&contNames, "container-name", "The name of a container that must be running on the Docker server (can specify multiple times)")
